...
class RoPE(nn.Module):
    def __init__(self, d_model: int, max_seq_len: int):
        ...
        seq = torch.arange(max_seq_len, dtype=torch.long)
        idx_theta = torch.einsum('s,d->sd', seq, theta)
        idx_theta2 = torch.cat([idx_theta, idx_theta], dim=-1)
        cos_emb = idx_theta2.cos().to(torch.bfloat16)
        sin_emb = idx_theta2.sin().to(torch.bfloat16)
    ...
    def forward(self, x: torch.Tensor, start_pos: int = 0) -> torch.Tensor:
        ...
        # Slice precomputed cos/sin for this sequence
...
class MLA(nn.Module):
    def __init__(self, config: Config):
        ...
        self.q_rope = RoPE(self.rope_head_dim, config.max_seq_len)
        self.k_rope = RoPE(self.rope_head_dim, config.max_seq_len)
